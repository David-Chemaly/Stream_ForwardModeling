...
def log_likelihood_GMM(params, dict_data, type_samples='data'):
    ...
    x_model, y_model = model_stream(params)
...
def model_stream(params):
    ...
    distribution_func = ms.FardalStreamDF(gala_modified=True)
    gen = ms.MockStreamGenerator(distribution_func, H)

    dt    = 1 * u.Myr
    steps = (t_end * u.Gyr / dt.to(u.Gyr)).value
    stream, _ = gen.run(w0, (10**logm)*u.Msun, dt=dt, n_steps=steps)

    x, y, _ = stream.xyz.value # kpc

    return x, y


def model_orbit(params):
    # Unpack parameters
    logM, Rs, \
    _, \
    pos_init_x, pos_init_y, pos_init_z, \
...
if __name__ == "__main__":
    ...
    n_eff = 10000
    seed =  4773 #np.random.randint(0, 10000) #4773, 1701
    np.random.seed(seed)
    sigma = 3
    nlive = 1200
    ...
    x_data, y_data = model_orbit(theo_params)
    ...
    for i in range(10):
        ...
        plt.scatter(x_model, y_model, c='b', s=1)
